...
Программа должна вывести в соответствии с указанным алгоритмом строки, количество которых
равно количеству разных букв в строке, которая получается путем конкатенации введенного
...
key_word = f'{input()} запретил букву '
ru_dict = 'абвгдежзийклмнопрстуфхцчшщъыьэюя'

for ch in ru_dict:
    if ch in key_word:
        print(key_word, ch)
        key_word = key_word.replace(ch, '').strip()
        if '  ' in key_word:
